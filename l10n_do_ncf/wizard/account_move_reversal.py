...
class AccountMoveReversal(models.TransientModel):
    ...
    #             xml_id = action_map[inv.move_type]
    #     if xml_id:
    #         result = self.env.ref("l10n_do_ncf.%s" % xml_id).read()[0]
    #         invoice_domain = safe_eval(result["domain"])
    #         invoice_domain.append(("id", "in", created_inv))
    #         result["domain"] = invoice_domain
    #         return result
    ...
    def _get_defaul_type(self):        
        fiscal = self.env['account.fiscal.type'].search([('prefix','=','E34'),('type','=','out_refund')])
        for rec in  fiscal:
            return rec.id
    ...
    def _prepare_default_reversal(self, move):
        ...
        if self.is_fiscal_refund:
            res.update({
                ...
                'fiscal_type_id': self._get_defaul_type() if move.l10n_do_sequence_prefix and move.l10n_do_sequence_prefix.startswith('E') and move.move_type == 'out_invoice' else False
            })
        return res