...
class AccountFiscalSequence(models.Model):
    ...
    def action_confirm(self):
        ...
        action = self.sudo().env.ref(
            "l10n_do_ncf.account_fiscal_sequence_validate_wizard_action"
        ).read()[0]
        action["context"] = {
            "default_name": msg,
    ...
    def get_fiscal_number(self):
        ...
        self.flush_recordset(["number_next_actual", "state", "company_id", "fiscal_type_id"])
        self.env.cr.execute(
            "SELECT id FROM account_fiscal_sequence WHERE id = %s FOR UPDATE",
            [self.id],
        )
        self.invalidate_recordset(["number_next_actual", "state"])
        ...
        if self.sequence_remaining > 0:
            ...
            if (self.sequence_remaining - 1) < 1:
                ...
                if queue_sequence_id:
                    queue_sequence_id.flush_recordset(["state", "company_id", "fiscal_type_id"])
                    self.env.cr.execute(
                        "SELECT id FROM account_fiscal_sequence WHERE id = %s FOR UPDATE",
                        [queue_sequence_id.id],
                    )
                    queue_sequence_id.invalidate_recordset(["state", "number_next_actual"])
