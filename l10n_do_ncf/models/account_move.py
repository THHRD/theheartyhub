...
class AccountMove(models.Model):
    ...
    #     """ Validates an invoice with Special Regimes fiscal type
    ...
    def _post(self, soft=True):
                    ...
                    if (
                        ...
                        raise UserError(
                            _(
                                ...
                                u"RD$250,000.00 "
                                u"the customer should have RNC or ID"
                                u"for make invoice"
                            )
                        )
        ...
        for inv in self:
            print('ref?', inv.ref)
            print('inv.is_l10n_do_fiscal_invoice', inv.is_l10n_do_fiscal_invoice)
            print('inv.fiscal_type_id.assigned_sequence',  inv.fiscal_type_id.assigned_sequence)
            print('inv.is_invoice()', inv.is_invoice())
            print('inv.state == "posted"', inv.state)
            print('inv date', inv.invoice_date)
    ...
    def action_invoice_cancel(self):
        ...
        if fiscal_invoice:
            action = self.env.ref(
                'l10n_do_ncf.action_account_move_cancel'
            ).read()[0]
